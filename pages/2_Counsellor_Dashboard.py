...
ranked_df = get_all_cases_ranked_guarded()
...
display_df = ranked_df[[
    ...
    "composite_score", "classification", "pending_confirmation"
]].copy()

display_df["Risk Level"] = display_df["classification"].apply(fmt_badge)
display_df["Confirmation"] = display_df["pending_confirmation"].apply(
    lambda pending: "⏳ Pending Confirmation" if pending else ""
)
...
st.dataframe(
    display_df[["Case ID", "Crime Category", "State", "District", "Channel", "Risk Score (0–100)", "Risk Level", "Confirmation"]],
...
if pending_confirmation:
    st.warning(risk_explanation["message"])
st.info(get_recommended_intervention(classification))
